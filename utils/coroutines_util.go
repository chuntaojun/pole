...
func Go(ctx *common.ContextPole, work func(ctx *common.ContextPole)) {
	go work(ctx)
}

func DoTimerSchedule(ctx *common.ContextPole, work func(), delay time.Duration, supplier func() time.Duration) {
	go func() {
...
func (htw *HashTimeWheel) Start() {
	Go(common.NewCtxPole(), func(ctx *common.ContextPole) {
		for {
			select {
