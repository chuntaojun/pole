...
type KVStorage interface {
	...
	RangeWithStartAndEnd(start, end []byte, call func(key, value []byte) error) error

	Range(func(key, value []byte) error) error
...
func (mks *memoryKVStorage) Read(key []byte) ([]byte, error) {
	defer mks.lock.RUnlock()
	mks.lock.RLock()
	return mks.reference.Load().(map[string][]byte)[string(key)], nil
}

func (mks *memoryKVStorage) ReadBatch(keys [][]byte) ([][]byte, error) {
	defer mks.lock.RUnlock()
	mks.lock.RLock()
...
func (mks *memoryKVStorage) Write(key []byte, value []byte) error {
...
func (mks *memoryKVStorage) WriteBatch(keys [][]byte, values [][]byte) error {
	defer mks.lock.Unlock()
	mks.lock.Lock()
	for i, key := range keys {
		k := string(key)
...
func (mks *memoryKVStorage) Delete(key []byte) error {
	defer mks.lock.Unlock()
	mks.lock.Lock()
...
func (mks *memoryKVStorage) DeleteBatch(keys [][]byte) error {
	defer mks.lock.Unlock()
	mks.lock.Lock()
...
func (mks *memoryKVStorage) RangeWithStartAndEnd(start, end []byte, call func(key, value []byte) error) error {
	defer mks.lock.RUnlock()
	mks.lock.RLock()

	mks.sortKeys.FindMin()

	return nil
}

func (mks *memoryKVStorage) Range(f func(key, value []byte) error) error {
	defer mks.lock.RUnlock()
	mks.lock.RLock()

	container := mks.reference.Load().(map[string][]byte)

	return mks.sortKeys.Range(func(n *node) error {
		return f([]byte(n.val), container[n.val])
	})
}
...
func (bks *badgerKVStorage) Read(key []byte) ([]byte, error) {
...
func (bks *badgerKVStorage) ReadBatch(keys [][]byte) ([][]byte, error) {
	vs := make([][]byte, 0, 0)
	txn := bks.kv.NewTransaction(false)
	for _, key := range keys {
		item, err := txn.Get(key)
...
func (bks *badgerKVStorage) Write(key []byte, value []byte) error {
...
func (bks *badgerKVStorage) WriteBatch(keys [][]byte, values [][]byte) error {
	txn := bks.kv.NewWriteBatch()
	for i, k := range keys {
...
func (bks *badgerKVStorage) Delete(key []byte) error {
...
func (bks *badgerKVStorage) DeleteBatch(keys [][]byte) error {
	txn := bks.kv.NewTransaction(true)
	for _, key := range keys {
		if err := txn.Delete(key); err != nil {
			txn.Discard()
...
func (bks *badgerKVStorage) RangeWithStartAndEnd(start, end []byte, call func(key, value []byte) error) error {
	return nil
}

func (bks *badgerKVStorage) Range(call func(key, value []byte) error) error {
	txn := bks.kv.NewTransaction(false)
	it := txn.NewIterator(badger.DefaultIteratorOptions)
	for it.Valid() {
		entry := it.Item()
		if err := entry.Value(func(val []byte) error {
			return call(entry.Key(), val)
		}); err != nil {
			return err
		}
	}
